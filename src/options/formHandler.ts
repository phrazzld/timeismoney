 ...
 */
export function sanitizeCurrencySymbol(input: string): string {
  if (typeof input !== 'string') {
    return '';
  }
 ...
 */
export async function loadForm(): Promise<void> {
  ...
  try {
    ...
    logger.error('Error loading options form:', (error as Error).message);
...
export function setupListeners(): void {
  ...
  const togglrBtn = document.getElementById('togglr');
  ...
  if (togglrBtn) {
    togglrBtn.addEventListener('click', toggleFormatting);
  }
}
...
export function saveOptions(): void {
  // Get form elements
  const currencySymbolEl = document.getElementById('currency-symbol') as HTMLInputElement;
  const currencyCodeEl = document.getElementById('currency-code') as HTMLInputElement;
  const frequencyEl = document.getElementById('frequency') as HTMLInputElement;
  const amountEl = document.getElementById('amount') as HTMLInputElement;
  const thousandsEl = document.getElementById('thousands') as HTMLSelectElement;
  const decimalEl = document.getElementById('decimal') as HTMLSelectElement;
  const debounceIntervalEl = document.getElementById('debounce-interval') as HTMLInputElement;
  const enableDynamicScanningEl = document.getElementById(
    'enable-dynamic-scanning'
  ) as HTMLInputElement;
  const debugModeEl = document.getElementById('enable-debug-mode') as HTMLInputElement;
  ...
  // Get form values
  const rawCurrencySymbol = currencySymbolEl.value.trim();
  const rawCurrencyCode = currencyCodeEl.value.trim().toUpperCase();
  const rawFrequency = frequencyEl.value;
  const rawAmount = amountEl.value;
  const rawThousands = thousandsEl.value;
  const rawDecimal = decimalEl.value;
  const rawDebounceIntervalStr = debounceIntervalEl.value;
  const enableDynamicScanning = enableDynamicScanningEl.checked;
 ...
 */
function loadSavedOption(elementId: string, value: any, decimal: string = 'dot'): void {
 ...
 */
function formatIncomeAmount(x: string | number, decimal: string): string {
  if (decimal === 'dot') {
