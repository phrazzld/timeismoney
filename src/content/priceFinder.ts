...
/**
 * Price find result interface
 */
interface PriceFindResult {
  text: string;
  culture: string;
 ...
 * @param {Settings} settings - User settings that may override the detected culture
 * @returns {string} Culture string (e.g., 'en-US', 'de-DE')
 */
export const detectCultureFromText = (text: string, settings: Settings | null = null): string => {
  ...
  if (settings && (settings.currencyCode || settings.currencySymbol)) {
    ...
    if (formatGroup && CURRENCY_FORMATS[formatGroup]) {
      return (CURRENCY_FORMATS[formatGroup] as CurrencyFormat).localeId;
    }
  }
 ...
 */
export const mightContainPrice = (text: string): boolean => {
 ...
 * @param {Settings} [settings] - Optional format settings from user preferences
 * @returns {PriceFindResult | null} Object with price finding data, or null if invalid input
 */
export const findPrices = (text: string, settings: Settings | null = null): PriceFindResult | null => {
  // Quick validation
  if (!text) return null;
  ...
  let reversePattern: RegExp | null = null;
 ...
 */
export const determineCulture = (settings: Settings): string => {
  ...
  for (const formatKey in CURRENCY_FORMATS) {
    if ((CURRENCY_FORMATS[formatKey] as CurrencyFormat).localeId === simplifiedLocale) {
...
const patternCache: Record<string, Map<string, RegExp>> = {
  match: new Map(),
  reverse: new Map(),
  thousands: new Map(),
  decimal: new Map(),
 ...
 */
export const buildThousandsString = (delimiter: string): string => {
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const buildDecimalString = (delimiter: string): string => {
  // Simulate the cache mechanism for tests
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const getLocaleFormat = (currencySymbol?: string, currencyCode?: string): CurrencyFormat => {
  ...
  return (CURRENCY_FORMATS[formatGroup] || CURRENCY_FORMATS.US) as CurrencyFormat;
};
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const buildNumberPattern = (thousandsString: string, decimalString: string): string => {
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const buildSymbolBeforePattern = (escapedSymbol: string, currencyCode: string | undefined, numberPattern: string): string => {
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const buildSymbolAfterPattern = (escapedSymbol: string, currencyCode: string | undefined, numberPattern: string): string => {
  const symbolPart = escapedSymbol ? `(${escapedSymbol})` : currencyCode ? `(${currencyCode})` : '';
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const buildCurrencyCodePatterns = (currencyCode: string | undefined, numberPattern: string): string[] => {
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const buildMatchPattern = (currencySymbol?: string, currencyCode?: string, thousandsString?: string, decimalString?: string): RegExp => {
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const detectFormatFromText = (text: string): CurrencyFormat | null => {
  if (!text || typeof text !== 'string') return null;

  for (const [symbol, formatGroup] of Object.entries(CURRENCY_SYMBOL_TO_FORMAT)) {
    if (text.includes(symbol)) {
      return CURRENCY_FORMATS[formatGroup] as CurrencyFormat;
    }
  }
 ...
 * TODO: Remove when integration tests are updated to use service-based approach.
 */
export const getPriceInfo = (text: string, formatSettings: Settings | null = null): PriceInfo | null => {
  if (!text) return null;
