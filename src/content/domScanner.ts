 ...
 *
 * @returns {DomScannerState} A state object for managing DOM scanner state
 */
export const createDomScannerState = (): DomScannerState => {
  return {
 ...
 * @param {DomScannerState} state - The state object containing the debounceTimer reference
 * @returns {Function} The debounced function
 */
const debounce = (func: (...args: any[]) => void, wait: number, state: DomScannerState): (() => void) => {
...
/**
 * Callback type for walk function
 */
type WalkCallback = (textNode: Node, settings?: Settings) => void;
 ...
 * @param {WalkCallback} callback - Function to call on text nodes
 * @param {Settings} settings - The current extension settings
 * @param {WalkOptions} options - Optional settings for traversal
 */
export const walk = (
  node: Node,
  ...
  try {
    ...
    try {
      switch (node.nodeType) {
        ...
        case 11: // Document fragment
          ...
          if (settings?.debugMode && node.nodeType === 1) {
            debugTools.markPriceCandidate(node as Element);
          }
            ...
            try {
              ...
              try {
                ...
                logger.error(
                  ...
                  (amazonError as Error).message,
                  (amazonError as Error).stack
                );
              }
              ...
              if (!specialHandlerProcessed) {
                try {
                  ...
                  logger.error(
                    ...
                    (ebayError as Error).message,
                    (ebayError as Error).stack
                  );
                }
              }
              ...
              if (!specialHandlerProcessed) {
                try {
                  ...
                  logger.error(
                    ...
                    (attributeError as Error).message,
                    (attributeError as Error).stack
                  );
                }
              }
              ...
              logger.error('Error processing child node:', (childError as Error).message, (childError as Error).stack);
        ...
        case 3: // Text node
          try {
            ...
            logger.error('Error in node callback:', (callbackError as Error).message, {
              nodeContent: node?.nodeValue?.substring(0, 50) + '...',
              errorDetails: (callbackError as Error).stack,
            });
          }
          break;
        default:
      ...
      logger.error('Error accessing node properties:', (nodeTypeError as Error).message, (nodeTypeError as Error).stack);
    }
  } catch (error) {
    logger.error('Error in DOM walker:', (error as Error).message, (error as Error).stack);
  }
};

/**
 * MutationObserver constructor type
 */
 ...
 *
 * @param {WalkCallback} callback - Function to process text nodes (same as walk callback)
 * @param {WalkOptions} options - Optional configuration settings
 * @param {number} [debounceInterval] - Debounce interval in milliseconds. Higher values reduce CPU usage but may delay updates.
 * @param {DomScannerState} [state] - Optional state object to use. If not provided, uses the default state.
 * @param {MutationObserverConstructor} [ObserverClass] - Optional MutationObserver constructor for testing. If not provided, uses the global MutationObserver.
 * @returns {MutationObserver | null} The created observer instance
 */
export const observeDomChanges = (
  callback: WalkCallback,
  options: WalkOptions = {},
  debounceInterval: number = DEFAULT_DEBOUNCE_INTERVAL_MS,
  state: DomScannerState = defaultState,
  ObserverClass: MutationObserverConstructor | null = null
  ...
  try {
    ...
    const observer = new ActualObserver((mutations) => {
      try {
        ...
        logger.error('Error processing mutations:', (error as Error).message, (error as Error).stack);
      }
    });
    ...
    logger.error('Error creating MutationObserver:', (error as Error).message, (error as Error).stack);
    return null;
  }
};
 ...
 * @param {WalkCallback} callback - Function to process text nodes
 * @param {WalkOptions} options - Optional configuration
 * @param {number} [debounceInterval] - Debounce interval in milliseconds. Higher values reduce CPU usage but may delay updates.
 * @param {DomScannerState} [state] - Optional state object to use. If not provided, uses the default state.
 * @param {MutationObserverConstructor} [ObserverClass] - Optional MutationObserver constructor for testing. If not provided, uses the global MutationObserver.
 * @returns {MutationObserver | null} The active observer
 */
  ...
  try {
    ...
    getSettings()
      ...
      .catch((error) => {
        ...
        if (error && (error as Error).message === 'Extension context invalidated') {
          ...
          logger.warn('Debug mode initialization failed:', (error as Error).message);
    ...
    logger.error('Error starting MutationObserver:', (error as Error).message, (error as Error).stack);
    return null;
  }
};
 ...
 * @param {WalkCallback} callback - The function to call on text nodes
 * @param {WalkOptions} options - Optional settings for processing
 * @param {DomScannerState} state - The scanner state object
...
export const processMutations = (
  ...
  for (const mutation of mutations) {
    ...
    if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
      for (const node of mutation.addedNodes) {
        ...
        if (node.nodeType === 1) {
          ...
          while (current) {
            if ((current as Element).classList && (current as Element).classList.contains(CONVERTED_PRICE_CLASS)) {
    ...
    if (mutation.type === 'characterData' && mutation.target.nodeType === 3) {
      ...
      let current: Node | null = mutation.target.parentNode;
 ...
 *
 * @param {WalkCallback} callback - The function to call on each text node
 * @param {WalkOptions} options - Optional settings for the walk function
 * @param {DomScannerState} state - The scanner state object
 * @returns {void}
 */
export const processPendingNodes = (
  callback: WalkCallback,
  options: WalkOptions = {},
  state: DomScannerState = defaultState
): void => {
  try {
            ...
            for (const textNode of textNodesToProcess) {
              if (textNode && textNode.nodeType === 3) {
                try {
                  ...
                  logger.error('Error in debounced node callback:', (callbackError as Error).message, {
    ...
    logger.error('Error processing pending nodes:', (error as Error).message, (error as Error).stack);
    ...
    try {
      ...
      logger.error('Error adding performance timing on exception:', (perfError as Error).message);
    }
  }
};
 ...
 */
export const stopObserver = (state: DomScannerState = defaultState): boolean => {
  try {
    ...
    try {
      ...
      logger.error('Error clearing debounce timer:', (timerError as Error).message);
    }
    ...
    try {
      ...
      if (state.pendingTextNodes !== null && state.pendingTextNodes !== undefined) {
        if (typeof state.pendingTextNodes.clear === 'function') {
          ...
          state.pendingTextNodes = new Set<Node>();
        }
      } else {
        state.pendingTextNodes = new Set<Node>();
      }
    } catch (nodesError) {
      logger.error('Error clearing pending nodes:', (nodesError as Error).message);
      // Try to recover by resetting to new empty Sets
      try {
        state.pendingNodes = new Set<Node>();
        state.pendingTextNodes = new Set<Node>();
      } catch (recoveryError) {
        logger.error('Failed to recover from nodes error:', (recoveryError as Error).message);
      }
    }
    ...
    try {
      ...
      logger.error('Error resetting processing flag:', (flagError as Error).message);
    }
    ...
    try {
        ...
        if (typeof state.domObserver.disconnect === 'function') {
          try {
            ...
            logger.warn(
              ...
              (disconnectCallError as Error).message
            );
          }
        }
      ...
      logger.error('Error disconnecting observer:', (disconnectError as Error).message);
      // Attempt to clean up reference even if an error occurred
      try {
        state.domObserver = null;
      } catch (finalCleanupError) {
        // Last resort error handler
        logger.error('Error in final observer cleanup:', (finalCleanupError as Error).message);
      }
    }

    return false;
  } catch (error) {
    logger.error('Error stopping MutationObserver:', (error as Error).message, (error as Error).stack);
    return false;
  }
};
