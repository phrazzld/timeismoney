...
interface DebugState {
  ...
  lastText: string;
  lastDetectedPrice: string | null;
  debugPanel: HTMLElement | null;
  logEntries: LogEntry[];
  maxLogEntries: number;
  resetCounters(): void;
  addLogEntry(type: 'info' | 'warn' | 'error', message: string, data?: any): void;
 ...
 *
 * @returns {DebugState} Debug state object
 */
export const createDebugState = (): DebugState => {
  return {
     ...
     */
    addLogEntry(type: 'info' | 'warn' | 'error', message: string, data: any = null) {
 ...
 */
export const initDebugMode = (settings: Settings): void => {
 ...
 */
export const startScanTimer = (): void => {
 ...
 * End scanning timer
 */
export const endScanTimer = (): void => {
  if (!debugState.enabled) return;
 ...
 */
export const markPriceCandidate = (element: Element): void => {
 ...
 */
export const markTextProcessed = (
  textNode: Node,
  text: string,
  ...
  if (hasPotentialPrice) {
    ...
    const parentElement = (textNode as ChildNode).parentElement;
    if (parentElement) {
      parentElement.classList.add(DEBUG_HIGHLIGHT_CLASSES.DETECTED);
      debugState.addLogEntry('info', 'Potential price detected in text', { text });
    }
  }
 ...
 */
export const markConversionSuccess = (
  element: Element,
 ...
 */
export const markConversionFailure = (element: Element, text: string, reason: string): void => {
  if (!debugState.enabled || !element) return;
 ...
 */
export const recordConversionTime = (durationMs: number): void => {
  if (!debugState.enabled) return;
...
const createDebugPanel = (): void => {
  ...
  // Add event listeners
  const minimizeBtn = panel.querySelector('#tim-debug-minimize');
  if (minimizeBtn) {
  ...
  const clearBtn = panel.querySelector('#tim-debug-clear');
  if (clearBtn) {
  ...
  const closeBtn = panel.querySelector('#tim-debug-close');
  if (closeBtn) {
 ...
 */
const makeDraggable = (elementToDrag: HTMLElement, dragHandle: HTMLElement): void => {
  let pos1 = 0,
    pos2 = 0,
    pos3 = 0,
    pos4 = 0;
