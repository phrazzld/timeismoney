...
export class ShadowPriceBadge extends PriceBadge {
   ...
   */
  static supportsShadowDOM(): boolean {
   ...
   * @returns {ThemeInfo} Theme information
   */
  private _detectHostTheme(context: HTMLElement | null): ThemeInfo {
    try {
      if (!context) {
      ...
      logger.warn('Theme detection failed, using defaults:', (error as Error).message);
      return {
        isDark: false,
        textColor: '#333333',
        backgroundColor: '#ffffff',
   ...
   */
  private getShadowDOMContent(): string {
   ...
   */
  private _renderWithShadowDOM(): HTMLElement {
    try {
      // Create host element
      ...
      logger.error('Shadow DOM creation failed:', (error as Error).message);
  ...
  render(): HTMLElement {
    ...
    if (this.config.enableAccessibility && this.config.announceChanges) {
      ...
      import('../utils/accessibility.js')
        ...
        .catch((importError) => {
          logger.debug('Could not announce to screen reader:', (importError as Error).message);
        });
    }
  ...
  async destroy(): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Error destroying ShadowPriceBadge:', (error as Error).message);
      return false;
    }
  }
}
 ...
 * @returns {HTMLElement} The rendered badge element
 */
export function createShadowPriceBadge(
  originalPrice: string,
  timeDisplay: string,
  context: HTMLElement | null = null,
  config: Record<string, any> = {}
  ...
  try {
    ...
    logger.error('Error in createShadowPriceBadge factory function:', (error as Error).message);
    ...
    try {
      ...
      logger.error('Fallback badge creation also failed:', (fallbackError as Error).message);
      throw fallbackError;
    }
  }
}
