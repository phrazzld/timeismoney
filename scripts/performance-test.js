...
async function runPerformanceTests() {
  ...
  const reportFile = path.join(reportDir, `performance-report-${new Date().toISOString().split('T')[0]}.json`);
  const report = {
  ...
  try {
    ...
    for (const testPage of testPages) {
      ...
      const pageResults = await testPage(browser, testPage, extensionId);
 ...
 */
async function testPage(browser, testPage, extensionId) {
  ...
  const domStats = await page.evaluate(() => {
    return {
      ...
      textNodes: document.evaluate('//text()', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength,
...
function generateSummary(results) {
  ...
  results.forEach(result => {
    if (result.domStats.convertedPrices > 0) {
      ...
      if (firstConversion) {
        summary.timePerPrice[result.pageName] = firstConversion.duration / result.domStats.convertedPrices;
      }
    }
  });
...
function generateMarkdownSummary(report) {
  ...
  for (const result of report.results) {
    ...
    if (result.performanceResults.measures.length > 0) {
      ...
      const sortedMeasures = [...result.performanceResults.measures].sort((a, b) => b.duration - a.duration);
