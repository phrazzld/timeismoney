 ...
 *
 * Usage:
 * node scripts/move-hooks-inside-describe.js [path] [--dry-run] [--verbose]
 *
 * Options:
...
const searchPathArg = process.argv.find(
  (arg) => !arg.startsWith('--') && !arg.includes('node') && !arg.includes('scripts/move-hooks')
);
const searchPath = searchPathArg || 'src/__tests__';

// Hook patterns for regex matching
const HOOK_PATTERNS = {
  beforeEach: /^beforeEach\s*\(\s*(?:async)?\s*\(\s*\)\s*=>\s*\{([^}]*)\}\s*\);?/gm,
  afterEach: /^afterEach\s*\(\s*(?:async)?\s*\(\s*\)\s*=>\s*\{([^}]*)\}\s*\);?/gm,
  beforeAll: /^beforeAll\s*\(\s*(?:async)?\s*\(\s*\)\s*=>\s*\{([^}]*)\}\s*\);?/gm,
  afterAll: /^afterAll\s*\(\s*(?:async)?\s*\(\s*\)\s*=>\s*\{([^}]*)\}\s*\);?/gm,
};
 ...
 */
function processFiles(searchPath) {
  try {
...
function main() {
  ...
  const results = processFiles(searchPath);
